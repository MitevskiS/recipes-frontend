...
function EditRecipe() {
  ...
  const handleSave = () => {
    ...
    axios.put(`http://localhost:5000/api/recipes/${params.id}`, formData, {
      headers: {
        'x-access-token': `${userId}`,
      }}).then((response) => {
      navigate('/my-recipes');
  ...
  return (
    <>
      <div style={{
        ...
        <p style={{ color: '#96BB36', fontSize: '27px', fontFamily: 'cursive', width: '185px' }}>Edit Recipe</p>
        <div style={{
          width: '100%',
          border: '1px solid #EFEEEA',
          marginLeft: '10px',
        }} />
        <div className='edit-recipe-back-icon'>
          <Link to="/my-recipes">
      ...
      <div style={{
        ...
        <div style={{ width: '12%', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ width: '100%' }}>
            ...
            <label for="uploadImage" className='edit-recipe-button edit-recipe-upload-image-button'>Choose File</label>
          </div>
        </div>
        <div style={{ width: '88%' }}>
          <div style={{ display: 'flex' }}>
            <div style={{ display: 'flex', flexDirection: 'column', width: '50%', marginLeft: '50px' }}>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <input type="text" id="recipeTitle" name="recipeTitle" autoComplete="off" value={recipeTitle} onChange={(e) => setRecipeTitle(e.target.value)} style={{ background: '#F0EFEA', border: '1px solid #CECDCB', borderRadius: '3px', marginTop: '5px', padding: '5px' }} />
              </div>
              <div style={{ display: 'flex' }}>
                <div style={{ display: 'flex', flexDirection: 'column', width: '30%' }}>
                  <label htmlFor="category" style={{ color: 'orange', fontFamily: 'cursive' }}>Category</label>
                  <select name="category" id="category" value={recipeCategory} onChange={(e) => setRecipeCategory(e.target.value)} style={{ background: '#F0EFEA', border: '1px solid #CECDCB', borderRadius: '3px', marginTop: '5px', padding: '5px', color: 'grey' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', width: '30%', marginLeft: '22px' }}>
                  ...
                  <input type="number" id="preparationTime" name="preparationTime" autoComplete="off" value={prepTime} onChange={(e) => setPrepTime(parseInt(e.target.value))} style={{ background: '#F0EFEA', border: '1px solid #CECDCB', borderRadius: '3px', marginTop: '5px', padding: '5px' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', width: '30%', marginLeft: '22px' }}>
                  <label htmlFor="numberOfPeople" style={{ color: 'orange', fontFamily: 'cursive' }}>No. People</label>
                  <input type="number" id="numberOfPeople" name="numberOfPeople" autoComplete="off" value={numberOfPeople} onChange={(e) => setNumberOfPeople(parseInt(e.target.value))} style={{ background: '#F0EFEA', border: '1px solid #CECDCB', borderRadius: '3px', marginTop: '5px', padding: '5px' }} />
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <label htmlFor="shortDescription" style={{ color: 'orange', fontFamily: 'cursive' }}>Short Description</label>
                <textarea id="shortDescription" name="shortDescription" rows="3" value={shortDescription} onChange={(e) => setShortDescription(e.target.value)} style={{ background: '#F0EFEA', border: '1px solid #CECDCB', borderRadius: '3px', marginTop: '5px', padding: '5px', resize: 'none' }} />
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', width: '40%', marginLeft: '50px' }}>
              ...
              <textarea id="recipe" name="recipe" rows="10" value={description} onChange={(e) => setDescription(e.target.value)} style={{ background: '#F0EFEA', border: '1px solid #CECDCB', borderRadius: '3px', marginTop: '5px', padding: '5px', resize: 'none' }} />
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', width: '20%', marginLeft: '50px' }}>
            <button className='edit-recipe-button edit-recipe-button-submit' onClick={handleSave}>Save</button>
          </div>
        </div>
      </div>
    </>
